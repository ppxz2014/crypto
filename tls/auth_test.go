...
func TestLegacyTypeAndHash(t *testing.T) {
	...
	sigType, hashFunc, err = legacyTypeAndHashFromPublicKey(&testECDSAPrivateKey.(*ecdsa.PrivateKey).PublicKey)
	if err != nil {
	...
	sigType, hashFunc, err = legacyTypeAndHashFromPublicKey(testSM2PrivateKey.(*sm2.PrivateKey).PublicKey)
	if err != nil {
